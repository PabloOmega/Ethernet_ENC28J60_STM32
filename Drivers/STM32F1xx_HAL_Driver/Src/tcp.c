...
error_msg TCP_HTTP(uint32_t remoteAddress, uint16_t length){
    ...
    switch(tcpHeader.flags){
        ...
        case TCP_ACK_FLAG:
            ...
            if(!ind){
                currentTCB->connectionEvent = RCV_FACK;
                ind = 1;
            }
            break;
        case TCP_FIN_FLAG | TCP_ACK_FLAG:
            currentTCB->connectionEvent = RCV_FINACK;
            break;
        case TCP_SYN_FLAG | TCP_ACK_FLAG:
            currentTCB->connectionEvent = RCV_SYNACK;
            break;
        case TCP_RST_FLAG | TCP_ACK_FLAG:
            currentTCB->connectionEvent = RCV_RSTACK;
            break;
        case TCP_PSH_FLAG | TCP_ACK_FLAG:
            currentTCB->connectionEvent = RCV_ACK;
            break;
        //default:
            //while(1);            
...
//            if(tcpHeader.ack) currentTCB->connectionEvent = RCV_FINACK;
//            else currentTCB->connectionEvent = RCV_FIN;
//        }
//        else if(tcpHeader.rst){
//            if(tcpHeader.ack) currentTCB->connectionEvent = RCV_RSTACK;
//            else currentTCB->connectionEvent = RCV_RST;
//        }
//        else{
//            currentTCB->connectionEvent = RCV_ACK;
//            if(!ind){
...
//        }
       ...
       // currentTCB->fsmState = LISTEN;
       // TCP_FiniteStateMachine();
           HTTP_Snd(currentTCB, rcvPayloadLen); 
    

    return SUCCESS_COM;
}

error_msg HTTP_Snd(tcpTCB_t *tcbPtr, uint16_t len){
    switch(tcbPtr->connectionEvent){
        case RCV_SYN:
           tcpHeader.ackNumber      = ntohl(tcpHeader.ackNumber);
           tcpHeader.sequenceNumber = ntohl(tcpHeader.sequenceNumber);
           tcbPtr->remoteSeqno  = tcpHeader.sequenceNumber;
           tcbPtr->remoteAck    = tcbPtr->remoteSeqno + 1;
           tcbPtr->localLastAck = 0;
           tcbPtr->remoteWnd    = tcpHeader.windowSize;
           tcbPtr->mss          = 1024;
           tcbPtr->flags        = TCP_SYN_FLAG | TCP_ACK_FLAG;
           tcbPtr->timeout      = TCP_START_TIMEOUT_VAL;
           tcbPtr->timeoutReloadValue = TCP_START_TIMEOUT_VAL;
           tcbPtr->timeoutsCount      = TCP_MAX_SYN_RETRIES;
           TCP_Snd(tcbPtr); 
           break;
        case RCV_FACK: 
           break;
        case RCV_ACK:
           ...
           tcpHeader.sequenceNumber = ntohl(tcpHeader.sequenceNumber);
           tcbPtr->remoteSeqno  = tcpHeader.sequenceNumber;    
           tcbPtr->localSeqno   = tcpHeader.ackNumber;
           tcbPtr->remoteWnd    = tcpHeader.windowSize;
           ...
           tcbPtr->timeoutsCount      = TCP_MAX_SYN_RETRIES;
           if(len != 0){
               tcbPtr->remoteAck = tcbPtr->remoteSeqno + len;
               ETH_ReadBlock(&HTTP_RV,len);
           }
           else{
               tcbPtr->flags   = TCP_ACK_FLAG | TCP_FIN_FLAG;
               tcbPtr->remoteAck = tcbPtr->remoteSeqno;
               ind = 0;
           }
           TCP_Snd(tcbPtr); 
           if(HTTP_RV[0] == 'G' && tcpHeader.psh) {
               http_send = 1;
               tcbPtr->flags    = TCP_ACK_FLAG | TCP_PSH_FLAG;
           }
           else{
               break;
           }
           tcbPtr->bytesToSend  = strlen(mypage);
           tcbPtr->txBufferPtr  = mypage;
           TCP_Snd(tcbPtr); 
           break;
        case RCV_FINACK:
           break;
